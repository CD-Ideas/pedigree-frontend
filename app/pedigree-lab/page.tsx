            ...
            <Card className="h-full relative overflow-auto" style={{ minHeight: 560, padding: 0 }}>
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span dangerouslySetInnerHTML={{ __html: `<button onclick="try{var e=document.getElementById('pedigree-tree-container');if(!e)return;var u=0;try{var us=JSON.parse(localStorage.getItem('user')||'{}');u=us.id||0;}catch(e){}if(!u){alert('Please log in to save');return;}var overlay=document.createElement('div');overlay.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:99998;display:flex;align-items:center;justify-content:center';var modal=document.createElement('div');modal.style.cssText='background:#FAF7F2;border:2px solid #C9B29F;border-radius:12px;width:380px;max-width:90vw;overflow:hidden;box-shadow:0 8px 30px rgba(0,0,0,0.3)';modal.innerHTML='<div style=\"background:#1C1C1C;padding:12px 16px;color:#FAF7F2;font-family:var(--font-table);font-weight:700;font-size:14px\">Save Pedigree</div><div style=\"padding:20px\"><label style=\"display:block;font-family:var(--font-table);font-size:12px;font-weight:700;color:#4A4A4A;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.05em\">Name this pedigree</label><input id=\"save-name-input\" type=\"text\" value=\"Pedigree ${previewDisplayGens}G\" style=\"width:100%;padding:10px 12px;border:2px solid #C9B29F;border-radius:8px;font-size:14px;font-family:var(--font-table);color:#1C1C1C;background:#FAFAFA;outline:none;box-sizing:border-box\" /><div style=\"display:flex;gap:8px;margin-top:16px;justify-content:flex-end\"><button id=\"save-cancel\" style=\"padding:8px 20px;border-radius:8px;border:2px solid #C9B29F;background:transparent;color:#4A4A4A;font-family:var(--font-table);font-weight:700;font-size:12px;cursor:pointer\">Cancel</button><button id=\"save-confirm\" style=\"padding:8px 20px;border-radius:8px;border:none;background:#1C1C1C;color:#FAF7F2;font-family:var(--font-table);font-weight:700;font-size:12px;cursor:pointer\">Save</button></div></div>';overlay.appendChild(modal);document.body.appendChild(overlay);var inp=document.getElementById('save-name-input');inp.focus();inp.select();overlay.onclick=function(ev){if(ev.target===overlay){overlay.remove();}};document.getElementById('save-cancel').onclick=function(){overlay.remove();};inp.onkeydown=function(ev){if(ev.key==='Enter')document.getElementById('save-confirm').click();if(ev.key==='Escape')overlay.remove();};document.getElementById('save-confirm').onclick=function(){var name=inp.value.trim();if(!name){inp.style.borderColor='#ef4444';return;}overlay.remove();var t=document.createElement('div');t.textContent='Saving...';t.style.cssText='position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#1C1C1C;color:#FAF7F2;padding:8px 20px;border-radius:8px;font-size:12px;font-weight:700;z-index:99999;font-family:var(--font-table)';document.body.appendChild(t);import('html2canvas').then(function(m){var h=m.default;var tw=document.createElement('div');tw.style.cssText='position:fixed;left:-9999px;top:0;background:#fff;padding:8px;width:1400px';var cl=e.cloneNode(true);cl.style.transform='none';cl.style.minWidth='unset';cl.style.width='100%';tw.appendChild(cl);document.body.appendChild(tw);h(tw,{scale:1,backgroundColor:'#FAFAFA',useCORS:true,windowWidth:1400,logging:false}).then(function(cv){document.body.removeChild(tw);var img=cv.toDataURL('image/jpeg',0.7);fetch('/api/pedigree-folder/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:u,dogName:name,generation:${previewDisplayGens},image:img})}).then(function(r){t.remove();if(r.ok){var badge=document.getElementById('saved-pedigrees-badge');if(badge){var n=parseInt(badge.textContent||'0')+1;badge.textContent=String(n);badge.style.display='flex';}}});});});};}catch(x){console.error(x);}" style="background:#C9B29F;color:#1C1C1C;padding:4px 10px;border-radius:8px;border:2px solid #1C1C1C;cursor:pointer;font-weight:bold;font-size:12px;font-family:var(--font-table)">Save</button>` }} />
                  {/* Share Buttons */}

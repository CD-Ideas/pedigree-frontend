  ...
  return (
    ...
    >
        ...
        >
          <Card className="m-3 flex flex-col flex-1 overflow-hidden" style={{ padding: 0 }}>
            ...
            <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2" style={{ scrollbarWidth: "thin" }}>
              {searchLoading && (
                <p className="text-center text-xs py-4" style={{ color: "#4A4A4A" }}>
                  Searching...
                </p>
              )}
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[12px]" style={{ color: "#4A4A4A" }}>
                          <span style={{ color: dog.sex?.toUpperCase() === "FEMALE" ? "#9f1239" : "#1d5bbf" }}>{sexIcon(dog.sex)}</span> <span style={{ color: "#4A4A4A", fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)" }}>ID: {dog.dog_id}</span>
                        </p>
                      </div>
                      {/* Drag indicator */}
          ...
          {previewMode && previewTree.length > 0 ? (
            ...
            <Card className="h-full relative overflow-auto" style={{ minHeight: 560, padding: 0 }}>
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span dangerouslySetInnerHTML={{ __html: `<button onclick="window.__savePedigree()" style="background:#C9B29F;color:#1C1C1C;padding:4px 10px;border-radius:8px;border:2px solid #1C1C1C;cursor:pointer;font-weight:bold;font-size:12px;font-family:var(--font-table)">Save</button><script>window.__savePedigree=function(){try{var e=document.getElementById('pedigree-tree-container');if(!e)return;var u=0;try{var us=JSON.parse(localStorage.getItem('user')||'{}');u=us.id||0;}catch(e){}if(!u){alert('Please log in to save');return;}var ov=document.createElement('div');ov.id='save-overlay';ov.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:99998;display:flex;align-items:center;justify-content:center';ov.innerHTML='<div style="background:#FAF7F2;border:2px solid #C9B29F;border-radius:12px;width:380px;max-width:90vw;overflow:hidden;box-shadow:0 8px 30px rgba(0,0,0,0.3)"><div style="background:#1C1C1C;padding:12px 16px;color:#FAF7F2;font-weight:700;font-size:14px">Save Pedigree</div><div style="padding:20px"><label style="display:block;font-size:12px;font-weight:700;color:#4A4A4A;margin-bottom:6px;text-transform:uppercase;letter-spacing:0.05em">Name this pedigree</label><input id=sni type=text value="Pedigree ${previewDisplayGens}G" style="width:100%;padding:10px 12px;border:2px solid #C9B29F;border-radius:8px;font-size:14px;color:#1C1C1C;background:#FAFAFA;outline:none;box-sizing:border-box"><div style="display:flex;gap:8px;margin-top:16px;justify-content:flex-end"><button id=scn style="padding:8px 20px;border-radius:8px;border:2px solid #C9B29F;background:transparent;color:#4A4A4A;font-weight:700;font-size:12px;cursor:pointer">Cancel</button><button id=scf style="padding:8px 20px;border-radius:8px;border:none;background:#1C1C1C;color:#FAF7F2;font-weight:700;font-size:12px;cursor:pointer">Save</button></div></div></div>';document.body.appendChild(ov);var inp=document.getElementById('sni');setTimeout(function(){inp.focus();inp.select();},100);ov.onclick=function(ev){if(ev.target===ov)ov.remove();};document.getElementById('scn').onclick=function(){ov.remove();};inp.onkeydown=function(ev){if(ev.key==='Enter')document.getElementById('scf').click();if(ev.key==='Escape')ov.remove();};document.getElementById('scf').onclick=function(){var nm=inp.value.trim();if(!nm){inp.style.borderColor='#ef4444';return;}ov.remove();var t=document.createElement('div');t.textContent='Saving...';t.style.cssText='position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#1C1C1C;color:#FAF7F2;padding:8px 20px;border-radius:8px;font-size:12px;font-weight:700;z-index:99999';document.body.appendChild(t);import('html2canvas').then(function(m){var h=m.default;var tw=document.createElement('div');tw.style.cssText='position:fixed;left:-9999px;top:0;background:#fff;padding:8px;width:1400px';var cl=e.cloneNode(true);cl.style.transform='none';cl.style.minWidth='unset';cl.style.width='100%';tw.appendChild(cl);document.body.appendChild(tw);h(tw,{scale:1,backgroundColor:'#FAFAFA',useCORS:true,windowWidth:1400,logging:false}).then(function(cv){document.body.removeChild(tw);var img=cv.toDataURL('image/jpeg',0.7);fetch('/api/pedigree-folder/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:u,dogName:nm,generation:${previewDisplayGens},image:img})}).then(function(r){t.remove();if(r.ok){var b=document.getElementById('saved-pedigrees-badge');if(b){var n=parseInt(b.textContent||'0')+1;b.textContent=String(n);b.style.display='flex';}}});});});};}catch(x){console.error(x);}};<\/script>` }} />
                  {/* Share Buttons */}
            ...
            <Card className="h-full p-5 relative" style={{ minHeight: 560, backgroundImage: "radial-gradient(circle, #C9B29F33 1px, transparent 1px)", backgroundSize: "24px 24px" }}>
              ...
              >
                ...
                <span
                  className="text-[12px] uppercase tracking-widest mt-1 font-semibold"
                  style={{ color: riskColor(coi), fontFamily: "var(--font-table)" }}
                >
                ...
                onClick={async () => {
                  if (previewMode) {
                    ...
                    if (sireId || damId) {
                      ...
                      try {
                        const res = await fetch(`/api/dogs/pedigree-tree?sire_id=${sireId}&dam_id=${damId}&gens=5`);
                        if (res.ok) {
                          const data = await res.json();
      ...
      >
        ...
        <div
          className="rounded-full px-4 py-1 text-[12px] uppercase tracking-widest font-bold"
          style={{
            fontFamily: "var(--font-table)",
          ...
          >
            ...
            >
              ...
              <div className="flex items-end gap-3">
                <div>
                  <label
                    className="block text-[12px] uppercase tracking-widest font-semibold mb-1"
                    style={{ color: "#4A4A4A", fontFamily: "var(--font-table)" }}
                  >
                    🏷️ Prefix
                  </label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label
                    className="block text-[12px] uppercase tracking-widest font-semibold mb-2"
                    style={{ color: "#4A4A4A", fontFamily: "var(--font-table)" }}
                  >
                    👑 Owner
                  </label>
              ...
              <div>
                ...
                >
                  {publishForm.photoPreview ? (
                    ...
                    <>
                      <span className="text-2xl mb-1" style={{ color: "#4A4A4A" }}>
                        {"\uD83D\uDCF7"}
                      </span>
                      <span className="text-[12px]" style={{ color: "#4A4A4A" }}>
                    ...
                    >
                      ...
                      {vax.checked && (
                        <input
                          ...
                          className="rounded-lg px-2 py-1 text-[12px] outline-none transition-all"
                          style={{
                            background: "#FAFAFA",
                            border: "2px solid #C9B29F",
                            color: "#1C1C1C",
              ...
              <div>
                ...
                >
                  ...
                  {/* Date Wormed */}
                  <div>
                    <span className="text-[12px] uppercase tracking-widest font-semibold" style={{ color: "#4A4A4A", fontFamily: "var(--font-table)" }}>
                      Date Wormed
                    </span>
                    <input
                      type="date"
                    ...
                    <div className="flex gap-2 mt-1">
                      {[30, 60, 90].map((days) => (
                        <button
                          ...
                          className="rounded-lg px-3 py-1.5 text-[12px] font-bold uppercase tracking-widest transition-all"
                          style={{
                            fontFamily: "var(--font-table)",
                  ...
                  <button
                    ...
                    className="w-full rounded-lg py-2 text-[12px] font-bold uppercase tracking-widest transition-all hover:scale-[1.02]"
                    style={{
                      fontFamily: "var(--font-table)",
                      background: "#1C1C1C",
                      color: "#FAF7F2",
                    ...
                    {publishForm.journal.worming.map((entry, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[12px] font-semibold" style={{ color: "#1C1C1C", fontFamily: "var(--font-table)" }}>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className="text-[12px] hover:text-red-400 transition-colors"
                            style={{ color: "#4A4A4A" }}
                          >
                            {"\u2716"}
                          </button>
                ...
                return (
                  <div className="rounded-lg p-4" style={{
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-[12px] uppercase tracking-widest font-bold" style={{
                        color: "#9f1239",
                        fontFamily: "var(--font-table)",
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {/* Last Heat Date */}
                      <div>
                        <label className="block text-[12px] uppercase tracking-widest font-semibold mb-1"
                          style={{ color: "#4A4A4A", fontFamily: "var(--font-table)" }}>
                          Last Heat Date
                        </label>
                        <input
                    ...
                    {lastLoggedMonths !== null && lastLoggedMonths >= 12 && (
                      <div className="mt-2 rounded-lg px-3 py-2 flex items-center gap-2" style={{
                        ...
                        <p className="text-[12px]" style={{ color: "#fb923c", fontFamily: "var(--font-table)" }}>
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (treeData.length === 0) {
                      ...
                      if (sireId || damId) {
                        try {
                          const treeRes = await fetch(`/api/dogs/pedigree-tree?sire_id=${sireId}&dam_id=${damId}&gens=5`);
                          if (treeRes.ok) {
  ...
  if (dog) {
    return (
      ...
      >
        ...
        {/* Name */}
        <p
          className={`text-center font-bold truncate ${isSm ? "text-[12px]" : "text-[12px]"}`}
          style={{ color: titleColor, fontFamily: "var(--font-table)" }}
        >
          {displayName}
        </p>
...
export default function PedigreeLabPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center" style={{ background: "#EDE4D5", color: "#4A4A4A" }}>Loading...</div>}>
      <PedigreeLabInner />
    </Suspense>
  );
}

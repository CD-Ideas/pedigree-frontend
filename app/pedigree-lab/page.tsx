            ...
            <Card className="h-full relative overflow-auto" style={{ minHeight: 560, padding: 0 }}>
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span dangerouslySetInnerHTML={{ __html: `<button onclick="try{var e=document.getElementById('pedigree-tree-container');if(!e){alert('No pedigree');return;}var u=0;try{var us=JSON.parse(localStorage.getItem('user')||'{}');u=us.id||0;}catch(e){}if(!u){alert('Please log in to save');return;}var name=prompt('Name this pedigree:','Pedigree ${previewDisplayGens}G');if(!name)return;var t=document.createElement('div');t.textContent='Saving...';t.style.cssText='position:fixed;bottom:24px;right:24px;background:#1C1C1C;color:#FAF7F2;padding:12px 20px;border-radius:12px;font-size:12px;font-weight:700;z-index:99999;font-family:var(--font-table);box-shadow:0 8px 30px rgba(0,0,0,0.3);border:2px solid #C9B29F';document.body.appendChild(t);import('html2canvas').then(function(m){var h=m.default;var tw=document.createElement('div');tw.style.cssText='position:fixed;left:-9999px;top:0;background:#fff;padding:8px;width:1400px';var cl=e.cloneNode(true);cl.style.transform='none';cl.style.minWidth='unset';cl.style.width='100%';tw.appendChild(cl);document.body.appendChild(tw);h(tw,{scale:1,backgroundColor:'#FAFAFA',useCORS:true,windowWidth:1400,logging:false}).then(function(cv){document.body.removeChild(tw);var img=cv.toDataURL('image/jpeg',0.7);fetch('/api/pedigree-folder/save',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:u,dogName:name,generation:${previewDisplayGens},image:img})}).then(function(r){return r.json().then(function(data){t.remove();if(data.success){var savedId=data.id;var toast=document.createElement('div');toast.style.cssText='position:fixed;bottom:24px;right:24px;background:#1C1C1C;color:#FAF7F2;padding:16px 20px;border-radius:12px;font-size:12px;z-index:99999;font-family:var(--font-table);box-shadow:0 8px 30px rgba(0,0,0,0.3);border:2px solid #C9B29F;min-width:320px;max-width:400px';toast.innerHTML='<div style=\"display:flex;align-items:center;gap:8px;margin-bottom:10px\"><span style=\"font-size:16px\">✅</span><span style=\"font-weight:700\">Pedigree saved to My Saved Pedigrees</span></div><div style=\"display:flex;gap:8px\"><a href=/dashboard/pedigree-folder style=\"flex:1;text-align:center;padding:6px 12px;background:#C9B29F;color:#1C1C1C;border-radius:8px;font-weight:700;font-size:12px;text-decoration:none\">View My Saved Pedigrees</a><button id=undo-save style=\"padding:6px 12px;background:transparent;color:#FAF7F2;border:1px solid #C9B29F;border-radius:8px;font-weight:700;font-size:12px;cursor:pointer\">Undo</button><button id=close-toast style=\"padding:2px 8px;background:transparent;color:#FAF7F2;border:none;font-size:16px;cursor:pointer\">✕</button></div>';document.body.appendChild(toast);document.getElementById('close-toast').onclick=function(){toast.remove();};document.getElementById('undo-save').onclick=function(){fetch('/api/pedigree-folder/'+savedId,{method:'DELETE'}).then(function(){toast.remove();var ut=document.createElement('div');ut.textContent='Save undone';ut.style.cssText='position:fixed;bottom:24px;right:24px;background:#1C1C1C;color:#FAF7F2;padding:10px 20px;border-radius:8px;font-size:12px;font-weight:700;z-index:99999;font-family:var(--font-table)';document.body.appendChild(ut);setTimeout(function(){ut.remove();},2000);});};setTimeout(function(){if(toast.parentNode)toast.remove();},6000);}else{alert('Save failed');}});});});});}catch(x){alert(x);}" style="background:#C9B29F;color:#1C1C1C;padding:4px 10px;border-radius:8px;border:2px solid #1C1C1C;cursor:pointer;font-weight:bold;font-size:12px;font-family:var(--font-table)">Save</button>` }} />
                  {/* Share Buttons */}

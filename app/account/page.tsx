  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12 space-y-6">
      ...
      <div className="rounded-xl p-6" style={steelFrame}>
        ...
        {supportLoading ? (
          <p className="text-xs" style={{ color: "var(--text-muted)", fontFamily: "var(--font-table)" }}>Loading...</p>
        ) : supportMessages.length === 0 ? (
          <p className="text-xs" style={{ color: "var(--text-muted)", fontFamily: "var(--font-table)" }}>
            No support messages yet. Use the Contact/Support page to send a message.
          </p>
        ) : (
          <div className="space-y-3">
            {supportMessages.map((sm) => (
              <div
                key={sm.id}
                className="rounded-lg overflow-hidden"
                style={{ border: `1px solid ${sm.status === "replied" ? "rgba(34,197,94,0.25)" : "rgba(30,64,120,0.3)"}`, background: "rgba(11,17,32,0.5)" }}
              >
                {/* Header row - clickable */}
                <button
                  onClick={() => setExpandedMsg(expandedMsg === sm.id ? null : sm.id)}
                  className="w-full flex items-center justify-between px-4 py-3 text-left transition-colors hover:bg-white/[0.02]"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold truncate" style={{ color: "var(--text-primary)", fontFamily: "var(--font-table)" }}>
                        {sm.subject}
                      </p>
                      <span
                        className="flex-shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase"
                        style={{
                          background: sm.status === "replied" ? "rgba(34,197,94,0.1)" : "rgba(59,130,246,0.1)",
                          color: sm.status === "replied" ? "#22c55e" : "#3b82f6",
                          border: `1px solid ${sm.status === "replied" ? "rgba(34,197,94,0.2)" : "rgba(59,130,246,0.2)"}`,
                          fontFamily: "var(--font-table)",
                        }}
                      >
                        {sm.status === "replied" ? "Replied" : "New"}
                      </span>
                    </div>
                    <p className="text-[10px] mt-0.5" style={{ color: "#5a6a82", fontFamily: "var(--font-mono)" }}>
                      {new Date(sm.created_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                    </p>
                  </div>
                  <span className="text-xs ml-2 flex-shrink-0" style={{ color: "#5a6a82" }}>
                    {expandedMsg === sm.id ? "\u25B2" : "\u25BC"}
                  </span>
                </button>
                ...
                {/* Expanded content */}
                {expandedMsg === sm.id && (
                  <div className="px-4 pb-4 space-y-3" style={{ borderTop: "1px solid rgba(30,64,120,0.2)" }}>
                    {/* User's original message */}
                    <div className="pt-3">
                      <p className="text-[10px] uppercase tracking-wider font-semibold mb-1" style={{ color: "#5a6a82", fontFamily: "var(--font-table)" }}>
                        Your Message
                      </p>
                      <p className="text-xs leading-relaxed whitespace-pre-wrap" style={{ color: "var(--text-secondary, #b0bec5)", fontFamily: "var(--font-table)" }}>
                        {sm.message}
                      </p>
                    </div>
                    ...
                    {sm.admin_reply ? (
                      <div className="rounded-lg p-3" style={{ background: "rgba(34,197,94,0.05)", border: "1px solid rgba(34,197,94,0.15)" }}>
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <p className="text-[10px] uppercase tracking-wider font-semibold" style={{ color: "#22c55e", fontFamily: "var(--font-table)" }}>
                            Admin Reply
                          </p>
                          {sm.replied_at && (
                            <p className="text-[10px]" style={{ color: "#5a6a82", fontFamily: "var(--font-mono)" }}>
                              {new Date(sm.replied_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                            </p>
                          )}
                        </div>
                        <p className="text-xs leading-relaxed whitespace-pre-wrap" style={{ color: "var(--text-primary)", fontFamily: "var(--font-table)" }}>
                          {sm.admin_reply}
                        </p>
                      </div>
                    ) : (
                      <div className="rounded-lg p-3" style={{ background: "rgba(59,130,246,0.05)", border: "1px solid rgba(59,130,246,0.15)" }}>
                        <p className="text-xs" style={{ color: "#3b82f6", fontFamily: "var(--font-table)" }}>

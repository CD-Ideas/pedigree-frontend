...
export async function GET(req: NextRequest) {
  try {
    ...
    const script = [
      ...
      `c.execute("SELECT COUNT(*) FROM marketplace_ads " + where, params)`,
      `total = c.fetchone()[0]`,
      `sort_col = "created_at DESC" if "${sort}" == "newest" else "views DESC" if "${sort}" == "most_viewed" else "created_at DESC"`,
      `c.execute("SELECT id, user_id, category, title, description, price, photos, contact_phone, contact_email, contact_venmo, contact_paypal, dog_id, location, status, is_verified, is_paid, views, created_at, expires_at FROM marketplace_ads " + where + " ORDER BY " + sort_col + " LIMIT ? OFFSET ?", params + [${limit}, ${offset}])`,

...
export async function GET(req: NextRequest) {
  try {
    ...
    const script = [
      ...
      `c.execute("SELECT COUNT(*) FROM dogs d " + where.replace("registered_name", "d.registered_name").replace("photo_url", "d.photo_url").replace("UPPER(sex)", "UPPER(d.sex)").replace("color =", "d.color ="), params)`,
      `total = c.fetchone()[0]`,
      `sort_col = "${sortCol}"`,
      `sort_order = "${order}"`,
      `lim = ${limit}`,
      `off = ${offset}`,
      `c.execute("SELECT d.dog_id as id, d.registered_name as name, d.sex, d.color, d.birthdate as dob, d.registration_number as reg_number, d.photo_url as profile_image_url, d.sire_id, d.dam_id, d.view_count, s.registered_name as sire_name, dm.registered_name as dam_name FROM dogs d LEFT JOIN dogs s ON d.sire_id = s.dog_id LEFT JOIN dogs dm ON d.dam_id = dm.dog_id " + where.replace("registered_name", "d.registered_name").replace("photo_url", "d.photo_url").replace("UPPER(sex)", "UPPER(d.sex)").replace("color =", "d.color =") + " ORDER BY d." + sort_col + " " + sort_order + " LIMIT ? OFFSET ?", params + [lim, off])`,
      `dogs = []`,
      `for r in c.fetchall():`,
      `    d = dict(r)`,
      `    d["titles"] = []`,
